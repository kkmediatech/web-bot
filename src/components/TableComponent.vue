      ...
      async submitTurnstile() {
        ...
        if (window.turnstile) {
          ...
          try {
            ...
            const response = await fetch('https://your-server-url/verify-captcha', {
              method: 'POST',
              headers: {
            ...
            if (result.success) {
              // Redirect to Details page with the selected row data
              this.$router.push({
                name: 'Details',
                params: { rowData: this.selectedRow }
              });
            } else {
